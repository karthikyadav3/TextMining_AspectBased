...
"""
Created on Sun Sep 15 09:16:29 2019
...
class Data:
    def __init__(self, Category, Directory, label_dict, max_seq_len,
     flag_change_file_structure, negative_weight, positive_weight, neutral_weight, flag_use_sentiment_embedding):
        ...
        self.word_dict, self.word_dict_rev, self.embedding, aspect_list = LaptopPreprocess.load_data(
            self.Category,
            self.Directory,
            self.label_dict,
            self.max_seq_len,
    ...
    def parse_data(self, sentences):
        ...
        for sentence in sentences:
            ...
            count_len = 0
            for word in words:
                if (word in word_dict.keys() and count_len < max_seq_len):
                    data_tmp.append(self.word_dict[word])
                    binary_mask_tmp.append(1.0)
                    count_len = count_len + 1
    
            for _ in range(self.max_seq_len - count_len):
...
class Model:
    def __init__(self, batch_size, max_seq_len, nb_sentiment_label, nb_sentiment_for_word, embedding_size, nb_linear_inside,
        nb_lstm_inside, layers, epochs, LEARNING_RATE, WEIGHT_DECAY, flag_train, flag_use_sentiment_for_word, sess):
    ...
    def modeling(self):
        self.tf_X_train = tf.placeholder(tf.float32, \
            shape = [None, self.max_seq_len, self.embedding_size - int(self.flag_use_sentiment_for_word) * self.nb_sentiment_for_word])
...
#        self.saver.save(self.sess, '../ckpt/se-apect-v0.ckpt')
    ...
    def evaluate(self, data, flag_write_to_file, flag_train):
        ...
        if (flag_write_to_file):
            f_result = codecs.open('C:/karthik/TMP-ABSA/code/laptops/result/result.txt', 'w', 'utf-8')
            f_result.write('#---------------------------------------------------------------------------------------------------------#\n')
...
def main():
    ...
    model = Model(batch_size, max_seq_len, nb_sentiment_label, nb_sentiment_for_word, embedding_size, nb_linear_inside,
     nb_lstm_inside, layers, epochs, LEARNING_RATE, WEIGHT_DECAY, flag_train, flag_use_sentiment_for_word,sess)
